...
def generate_html_report(analysis, chart_dir, out_path):
    _get_font()
    # Station table
    station_rows = []
    ...
    class App:
        ...
        def _run_thread(self, source):
            try:
                ...
                self.root.after(0, lambda: self._show_results(analysis))
                ...
                if self.opt_html.get():
                    ...
                    self.report_html_path = generate_html_report(analysis, out_dir, os.path.join(out_dir, "report.html"))
        ...
        def _clear_results(self):
            ...
            for tree in [self.station_tree, self.fail_tree]:
