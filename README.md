...
- **通用编码检测** — 不再依赖 SN 前缀（如 `BLKVUN`），适配所有 SN 格式（`BKVXUN`/`BKVVUN`/`BCYTUN` 等）
- **合并 v1 + v3** — 旧的 Format A（9行块）和新的 Format B（行级40项）统一一套解析器
- **ZIP 流式读取** — 不落盘，消除 os.walk 导致的 Windows 不确定性
- **站别路径检测** — 向上逐级查找 ATxx，不再硬编码深度
...
│                     │                                │
│ 📂 输出目录           │  ┌─ 📊站别 ──┬─ 🔍失败原因 ──┐  │
│ [____________] [选择]│  │AT01 98.5%✅│MIC1气密差值 FR│  │
│                     │  │AT02 100% ✅│左主1标压 R&B  │  │
│ ⚙️ 输出选项           │  │AT03 98.6%✅│MIC2气密差值 FR│  │
│ ☑ PNG ☑ HTML ☑ CSV  │  │AT04 98.0%✅│右主2标压 R&B  │  │
│ ☑ 自动打开            │  │AT05 99.2%✅│...           │  │
│                     │  │AT06 94.8%🔴│              │  │
│ [▶ 开始分析]         │  │AT07 94.1%🔴│              │  │
│                     │  └───────────┴──────────────┘  │
...
│                     │  [📄打开报告] [📂输出目录]       │
├─────────────────────┴────────────────────────────────┤
│ ✅ 完成！总数977台 PASS=954 FAIL=23 良率97.6%        │
└──────────────────────────────────────────────────────┘
```
...
### v4.0 (2026-06-16)
- **合并 v1 + v3**：统一 Format A/B 解析器到主工具
- **通用编码检测**：不再依赖 SN 前缀（如 BLKVUN），只需检测到 "SN" 字面
- **站别路径检测**：向上逐级查找 ATxx，不硬编码深度
- **ZIP 直读流**：消除 extractall + os.walk 导致的 Windows 不确定性
